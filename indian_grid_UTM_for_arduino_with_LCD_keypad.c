...
double round_double(double var)
{
	 ...
	 // the 100000 is for 5 digits round off
		...
		double value = (round(var * 100000));
		return value / 100000;
}

double DegMinSecToDegree(String input){
	char separator[] = ",";
	String token;
	double result=0.0;
	double temp=0.0;
	...
	while(token != NULL)
	{
		temp=token.toDouble();
		result+=temp/divider;
		divider= divider*60.0; 
		toke/n = strtok(NULL, separator);
	}
	result= round_double(result);
	return (result);
	...
	while (1){
		...
		if (keypressed != NO_KEY) {
			...
			if ((keypressed >= '0' && keypressed <= '9') || (keypressed == '.')){     
				...
				if (inputString.length() > 0) {
					inputDouble = DegMinSecToDegree(inputString);
					inputString = "";// clear input
					keypressed = NO_KEY;
					// i dont think this statement is useful as
					// anyways it is going to rounded off
					// so do consider that
					printString = inputDouble;
					print_stuff_2nd_row();
					// a delay of 1 sec do take this off when u dont want this
					// delay(1000);
					return (inputDouble);
				}
			}
